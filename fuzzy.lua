...
M.cropInput = function(rangeMin, rangeMax)
	return function(x)
		return _min(_max(x, rangeMin), rangeMax)
	end
end

M.cropOutput = function(rangeMin, rangeMax)
	return function(x)
		return _min(_max(x, rangeMin), rangeMax)
	end
end
...
M.solver = function()
	...
	-- fuzzy set
	s_out.F = function(arg)
		assert(type(arg)=='table')
		local name, range = unpack(arg)
		...
		local crop = M.cropInput(unpack(range))
		local cropValue = M.cropOutput(0, 1)
